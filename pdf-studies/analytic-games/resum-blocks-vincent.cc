...
double KLL(double mu2, double mu02, double Gamma, double alphas){
    ...
    double beta1 = 16*pi*pi*b1;
    double beta2 = 64*pi*pi*pi*b2;
    
    double Gamma0 = Gamma*A0;
    
    double l = 1.+beta0/4./pi*alphas*log(mu02/mu2);
    double alphasmu0=0.0;
...
double KNLL(double mu2, double mu02, double Gamma, double gamma0, double alphas){
    double Ci = 1.;
    double A0 = 4.;
    double A1 = 4.*CA*(67./9.-pi*pi/3.)-80./9.*TR*nf;
    double beta0 = 4*pi*b0;
    double beta1 = 16*pi*pi*b1;
    double beta2 = 64*pi*pi*pi*b2;
...
double KNNLL(double mu2, double mu02, double Gamma, double gamma0, double gamma1, double alphas){
    ...
    double A0 = 4.;
    double A1 = 4.*CA*(67./9.-pi*pi/3.)-80./9.*TR*nf;
    double A2 = 4.*CA*CA*(245./6.-134.*pi*pi/27.+11.*pow(pi,4.)/45.+22.*Zeta3/3.)+32.*CA*TR*nf*(-209./108.+5.*pi*pi/27.-7.*Zeta3/3.)+4.*CF*TR*nf*(16*Zeta3-55./3.)-64./27.*TR*TR*nf*nf;
    double beta0 = 4*pi*b0;
    double beta1 = 16*pi*pi*b1;
    double beta2 = 64*pi*pi*pi*b2;
...
//     double Ci = 1.;
    double A0 = 4.;
//     double beta0 = 11./3.*CA-4./3.*TR*nf;
...
//     double Ci = 1.;
    ...
    double A1 = 4.*CA*(67./9.-pi*pi/3.)-80./9.*TR*nf;
    double beta0 = 4*pi*b0;
//     double beta1 = 34./3.*CA*CA-4.*TR*nf*(CF+5./3.*CA);
